...
async function main() {
  ...
  const token = jwt.sign({ userId: user.id, role: user.role }, process.env.JWT_SECRET || '4239c55b47e74a4b588639ca5ab62768f5ba44ae62a52', { expiresIn: '1d' });
  ...
  if (listData.data && listData.data.length > 0) {
    console.log("First saved job ID:", listData.data[0].job.id, "Matches:", listData.data[0].job.id === job.id);
  }
}
