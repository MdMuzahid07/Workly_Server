...
import { pushService, pushCircuitBreaker } from '../services/push.service.js';
...
async function checkPushReceipts(): Promise<void> {
  ...
  for (const chunk of receiptIdChunks) {
    const receipts = await pushCircuitBreaker.execute(() =>
      ...
      if (receipt.status === 'error') {
        ...
        if (errorCode === 'DeviceNotRegistered') {
          ...
          if (record?.pushToken) {
            ...
            console.log(`[PushReceiptJob] Deactivated dead token: ${record.pushToken}`);
          }
        }
